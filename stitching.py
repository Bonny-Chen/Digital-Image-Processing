...
# img3 = cv2.drawMatchesKnn(img1,kp1,frame,kp2,good,None,flags=cv2.DrawMatchesFlags_NOT_DRAW_SINGLE_POINTS)
# print(matches1)
...
def knn(a,keypointlength,threshold):
    ...
    for j in range(keypointlength):
        ...
        dis1=new[0]    # min 
        # print("min",dis1)
        dis2=new[1]    # second min
        itemindex1 = old.index(dis1)   #index 
        itemindex2 = old.index(dis2)          

        if dis1 < threshold*dis2: 
            bestmatch[index]=itemindex1
            distance[index]=dis1
            img1index[index]=j
            index=index+1
            verybest.append([dis1+dis2])
    return verybest
